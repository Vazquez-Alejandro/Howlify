...
from app_data import (
    calc_result_score,
    get_result_badge,
    sort_results_intelligent,
    password_strength,
    normalize_phone,
    get_base64_logo,
    play_wolf_sound,
    contar_cazas_activas,
    es_plan_business,
    _fmt_money,
    _fmt_pct,
    _safe_int,
    collect_live_business_ops,
    get_price_history_stats_by_caza,
    get_price_history_series_by_caza,
    get_monitor_rules_map,
    delete_monitor_rule,
    monitor_status,
    compliance_pct_from_series,
    _make_histogram_df,
    save_user_whatsapp,
)
...
def get_image_base64(path):
    ...
    if os.path.exists(path):
        try:
            ...
            logger.error(f"⚠️ Error al procesar imagen Base64: {e}")
            return None
    return None
        ...
        with c1:
            if st.button("💾 Guardar", width='stretch', type="primary"):
                try:
                    ...
                    logger.info("👉 RESULTADO SUPABASE:", res.data, " | ID BUSCADO:", rid)
...
def run_manual_hunt(b, headless=True):
    ...
    logger.info(f"[!!!] FUENTE DETECTADA: '{src}'")

    if src == "airbnb":
        logger.info("🏠 [Dispatcher] Airbnb detectado. Saltando a hunt_airbnb...")
        return hunt_airbnb(url, precio)
    
    # Si llega acá, es porque no entró al if de arriba
    logger.info(f"🌐 [Dispatcher] No es Airbnb. Derivando a hunt_offers para {src}...")
...
def upsert_monitor_rule(
    ...
    try:
        ...
        logger.error("[upsert_monitor_rule] error:", e)
        return False
        ...
        if st.button("Entrar", width='stretch', type="primary", key="l_submit"):
            ...
            if user:
                ...
                try:
                    ...
                    logger.error("[login profile] error:", e)
