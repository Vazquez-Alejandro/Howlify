"""
howlify/tasks.py — Tareas Celery: el ÚNICO worker de Howlify.
Reemplaza engine/worker.py (standalone) y engine/engine.py (APScheduler).
"""
...
def _enviar_alerta(profile: dict, producto: str, oferta: dict, precio: float):
    ...
    if plan_allows_whatsapp(profile.get("plan", "")):
...
def hunt_all_user_task(self, user_id: str):
    """Busca ofertas para todas las cacerías activas de un usuario."""
    if not supabase:
        raise RuntimeError("Supabase no configurado")
...
def vigilar_ofertas_task():
    """Tick principal: chequea频率, busca ofertas, evalúa alertas, guarda historial."""
    if not supabase:
        ...
        try:
            ...
            if precio_actual <= precio_limite:
                from services.alerts_service import (
                    obtener_ultima_alerta, too_soon, obtener_contacto_usuario,
                    enviar_alerta_por_canal, guardar_alerta,
                )
                prev_res = supabase.table("alertas_enviadas") \
                    .select("*").eq("caza_id", caza_id) \
